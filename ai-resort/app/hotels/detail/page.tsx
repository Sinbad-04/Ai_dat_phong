...
function DetailInner() {
  ...
  const compareRows = showCompare
    ...
    : [];
  const minPrice = compareRows.length ? Math.min(...compareRows.map((r) => r.price)) : 0;
          ...
          {showCompare && (
            <section className="mt-8">
              ...
              <div className="mt-3 overflow-hidden rounded-2xl border border-teal/12">
                {compareRows.map((r) => {
                  const cheapest = r.price === minPrice;
                  return (
                    <div
                      key={r.platform}
                      className={`flex items-center justify-between gap-3 px-4 py-3 ${
                        r.ours ? "bg-jade/8" : "border-t border-teal/8"
                      }`}
                    >
                      <div className="flex items-center gap-2">
                        <span className={`text-sm ${r.ours ? "font-semibold text-teal" : "text-ink/75"}`}>
                          {r.platform}
                        </span>
                        {cheapest && (
                        ...
                        {r.estimated && (
                          <span className="rounded-full bg-ink/8 px-2 py-0.5 text-[10px] text-ink/50">ước tính</span>
                        )}
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="font-mono text-sm text-teal">{money(r.price, r.currency)}</span>
                        {r.ours ? (
              ...
              <p className="mt-2 text-xs text-ink/50">
                ...
                bấm “Kiểm tra” để mở đúng trang tìm kiếm thật của nền tảng đó và đối chiếu. (Muốn hiện
                giá thật tự động từ Booking/Agoda cần tích hợp Travelpayouts — xem README.)
              </p>
            </section>
          )}
        </div>

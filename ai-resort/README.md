...
## 13. So sánh giá đa nền tảng (metasearch nhẹ) — mô hình trung gian

Trang chi tiết khách sạn có mục **"So sánh giá các nền tảng"**:
- **Chúng tôi (LiteAPI):** giá **thật**, đặt + thanh toán được ngay tại web.
- **Booking.com / Agoda / Traveloka / Google Hotels:** hiện **giá ước tính minh hoạ** (gắn nhãn "ước tính"), kèm nút **"Kiểm tra"** mở đúng **trang tìm kiếm thật** của nền tảng đó theo tên khách sạn + ngày → khách tự đối chiếu.

### Vì sao chỉ ước tính?
Các OTA (Traveloka, Agoda, Booking…) **không mở API giá công khai** cho lập trình viên cá nhân — muốn lấy giá thật phải là **đối tác/affiliate** (duyệt thương mại, vài tuần). Scrape giá là **vi phạm điều khoản**, không nên dùng. Nên bản này dùng **link tìm kiếm thật** (hợp lệ, không cần API) + ước tính có nhãn rõ ràng.

### Nâng lên giá thật (bước tiếp theo)
Cách hợp pháp để hiện giá thật **inline** từ nhiều OTA là dùng một **aggregator/affiliate** mở cho dev:
- **Travelpayouts / Hotellook** — miễn phí đăng ký, gộp Booking.com + Agoda + nhiều bên; hợp cho app so sánh giá.
- Hoặc **Booking.com Demand API** (duyệt mức trung bình).

Khi tích hợp, thay phần ước tính trong `lib/pricecompare.ts` bằng lời gọi API thật (matching khách sạn theo tên/vị trí + ngày) và đổi `estimated: false`.

### File liên quan
- `lib/pricecompare.ts` — dựng các dòng so sánh (giá ta + ước tính OTA + link kiểm tra thật).
- `app/hotels/detail/page.tsx` — hiển thị bảng so sánh.
